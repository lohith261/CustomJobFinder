...
const FROM = "Job Hunter <noreply@jobhunter.app>";
const APP_URL = process.env.NEXTAUTH_URL || "https://custom-job-finder.vercel.app";
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
        ...
        <p style="color:#6b7280;margin-bottom:24px">Click the button below to verify your Job Hunter account. This link expires in 24 hours.</p>
...
export async function sendWelcomeEmail(email: string, name: string) {
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "Welcome to Job Hunter 🎯",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
        <h1 style="font-size:24px;font-weight:700;color:#1f2937;margin-bottom:8px">Welcome${name ? `, ${name}` : ""}! 🎯</h1>
        <p style="color:#6b7280;margin-bottom:16px">Your Job Hunter account is ready. Here's how to get started:</p>
        ...
        <a href="${APP_URL}" style="display:inline-block;background:#4f46e5;color:#fff;font-weight:600;padding:12px 24px;border-radius:8px;text-decoration:none">Open Job Hunter →</a>
      </div>
    `,
  });
}
...
export async function sendProActivationEmail(email: string, plan: string, endsAt: Date) {
  ...
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: "You're now on Job Hunter Pro ⚡",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
