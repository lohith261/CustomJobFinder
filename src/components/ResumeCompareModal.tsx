...
function WinBadge({ side }: { side: WinnerSide }) {
  if (side === "tie") {
    return (
      <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-700 px-2 py-0.5 text-[11px] font-semibold text-gray-500 dark:text-gray-300">
        Tie
      </span>
    );
  }
...
export default function ResumeCompareModal({ resumeA, resumeB, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-3xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Resume Comparison</h2>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] gap-2 px-6 pt-5 pb-3">
          ...
          >
            ...
            <p className="font-semibold text-gray-900 dark:text-gray-100 truncate" title={resumeA.name}>
              {resumeA.name}
            </p>
            {resumeA.isPrimary && (
          ...
          <div className="flex items-center justify-center">
            <div className="flex flex-col items-center gap-1">
              <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">vs</span>
              {overallWinner === "tie" && (
                <span className="text-[11px] text-gray-400 dark:text-gray-500 font-medium">Tied</span>
              )}
            </div>
          </div>
          ...
          >
            ...
            <p className="font-semibold text-gray-900 dark:text-gray-100 truncate" title={resumeB.name}>
              {resumeB.name}
            </p>
            {resumeB.isPrimary && (
              <span className="mt-1 inline-block text-[11px] font-semibold bg-blue-600 text-white px-2 py-0.5 rounded-full">
            ...
            return (
              ...
              >
                ...
                >
                  <p
                    className={`text-base font-bold ${
                      aWins ? "text-blue-700 dark:text-blue-400" : "text-gray-700 dark:text-gray-200"
                    }`}
                  >
                    {metric.valueA}
                  </p>
                  {aWins && (
                    <svg
                      className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 mx-auto mt-1"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
                ...
                <div className="flex flex-col items-center gap-1 min-w-[110px] text-center px-1">
                  ...
                  {metric.detail && (
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 leading-tight">{metric.detail}</p>
                  )}
                </div>
                ...
                >
                  <p
                    className={`text-base font-bold ${
                      bWins ? "text-violet-700 dark:text-violet-400" : "text-gray-700 dark:text-gray-200"
                    }`}
                  >
                    {metric.valueB}
                  </p>
                  {bWins && (
                    <svg
                      className="w-3.5 h-3.5 text-violet-500 dark:text-violet-400 mx-auto mt-1"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 rounded-b-2xl">
          {overallWinner === "tie" ? (
            ...
            <p className="text-sm text-center text-gray-700 dark:text-gray-200">
              <span
                className={`font-bold ${
                  overallWinner === "A" ? "text-blue-700 dark:text-blue-400" : "text-violet-700 dark:text-violet-400"
                }`}
              >
